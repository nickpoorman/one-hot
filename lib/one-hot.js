...
function OneHot(opts) {
  ...
  if (typeof opts.ignoreColumns === 'undefined') opts.ignoreColumns = [];
  this.ignoreColumns = createDictFromArray(opts.ignoreColumns);
...
OneHot.prototype.analyze = function(data, opts, cb) {
  // there is the case were just options is provided
  if (typeof data === 'object' && typeof opts !== 'function' && typeof cb !== 'function') {
    opts = data;
    data = undefined;
    cb = undefined;
  }
  ...
  if (typeof opts === 'function') { // opts is the callback
    cb = opts;
    opts = {};
  }
  if (typeof opts === 'undefined') opts = {}; // nothing was provided

  if (isArray(opts.ignoreColumns)) this.ignoreColumns = createDictFromArray(opts.ignoreColumns);
...
function _analyze(iv, cb) {
  ...
  this.indexes = _.union(this.indexes, getCategoricalIndices(iv, this.ignoreColumns));
  var indexes = this.indexes;
  var indexesFeatures = this.indexesFeatures;
 ...
 */
function getCategoricalIndices(array, ignoreColumns) {
  if (typeof ignoreColumns !== 'object') ignoreColumns = {};
  var indexes = [];
  for (var i = 0; i < array.length; i++) {
    if (typeof ignoreColumns[i] !== 'undefined') continue;
...
function createDictFromArray(array) {
  var dict = {};
