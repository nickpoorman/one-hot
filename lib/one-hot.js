 ...
 */
OneHot.prototype.buildColumnHeader = function(originalColumns) {
  ...
  var originalIndex;
  var originalColumName;
  var featureValue;

  // loop over what the encoded version of the iv would look like
  for (var encodedIndex = 0; encodedIndex < this.getNumberOfEncodedFeatures(); encodedIndex++) {
    originalIndex = this.getOriginalIndexFromEncodedIndex(encodedIndex); // TODO: this doesn't actually return the original index because we don't keep track of it yet
    originalColumName = originalColumns[originalIndex];

    // if the column was not one hot encoded
    if (!this.isEncodedIndexOneHot(encodedIndex)) {
      if (typeof originalColumName !== 'undefined') {
        header.push(originalColumName);
        continue;
      }
      header.push(null);
      continue;
    ...
    // the column was encoded
    if (typeof originalColumName !== 'undefined') {
      header.push(columnKeyToString(originalColumName, featureValue));
      continue;
    }
    header.push(columnKeyToString(originalIndex, featureValue));
  }
  return header;
}
...
/**
 * returns the number of encoded features
 * You should use this method and not the numberOfFeatures value on this object because it adjusts for the indexes expanded
 */
