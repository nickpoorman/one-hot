...
OneHot.prototype.buildColumnHeader = function(originalColumns) {
  ...
  for (var encodedIndex = 0; encodedIndex < this.getNumberOfEncodedFeatures(); encodedIndex++) {
    // if the column was not encoded
    // we need to get the internal index for which this encodedIndex belongs
    var internalIndex = getInternalIndexFromEncodedIndex(encodedIndex);
    if (typeof indexesFeatures[internalIndex] === 'undefined') {
      if (typeof originalColumns !== 'undefined' && typeof originalColumns[encodedIndex] !== 'undefined') {
        header.push(originalColumns[encodedIndex]);
        continue;
      }
      header.push(null);
      continue;
  ...
  // fill in anything that was not an original index
  ...
  for (var i = indexes.length - 1; i >= 0; i--) {
    ...
    featureValues = indexesFeatures[originalIndex];
  ...
  for (var originalIndex in this.indexesFeatures) {
    ...
    for (var featureValue in featureValues) {
      encodedIndex = featureValues[featureValue];
      header[getEncodedIndex.call(this, encodedIndex)] = columnKeyToString(originalIndex, featureValue);
    }
  }
  return header;
}
